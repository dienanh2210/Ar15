    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        void Update()
        {
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                touchWorldPos = Camera.main.ScreenToWorldPoint(Input.GetTouch(i).position);
                // touchWorldPos = new Vector2(Input.GetTouch(i).position.x, Input.GetTouch(i).position.y + distanceTouch);
